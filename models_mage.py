...
from taming_module.models.vqgan import VQModel
from omegaconf import OmegaConf
import numpy as np
import scipy.stats as stats

dataset_names = [
    'COCO_P', 'COCO_S', 'COCO_K', 'ADE', 'DPDD', 'LOL', 'SIDD', 
    'Color', 'RainDrop', 'DUTS', 'NYUv2_24k', 'Places2'
]
...
class MaskedGenerativeEncoderViT(nn.Module):
    ...
    def get_image_token(self, image):
        # print(image.shape)
        if len(image.shape) == 4: # N,C,H,W 
            image_token = self.vqgan.get_codebook_indices(image).flatten(1) # N,64
        elif len(image.shape) == 5: # N,K,C,H,W
            tokens = []
            num_img = image.shape[0]
            for i in range(num_img):
                token = self.vqgan.get_codebook_indices(image[i]).flatten(1) # k, 64
                token = token.flatten(0)
                tokens.append(token) 
            image_token = torch.stack(tokens) # N, K*64
        else: 
            assert len(image.shape) == 4 or len(image.shape) == 5

        return image_token

    def get_input(self, data, device):
        data_type = data['data_type']
        if data_type == 'CVF':
            B, C, H, W = data.shape
            # Image.fromarray(np.uint8(torch.clip(data[0].detach().cpu().squeeze().permute(1, 2, 0)*255, 0, 255).numpy())).save(f'test.png')
            sample_refer, sample_gt, refer, gt = data[...,:H//2,:H//2], data[...,:H//2,H//2:], data[...,H//2:,:H//2], data[...,H//2:,H//2:]
            refer = refer.to(device, non_blocking=True)
            gt = gt.to(device, non_blocking=True)
            sample_refer = sample_refer.to(device, non_blocking=True)
            sample_gt = sample_gt.to(device, non_blocking=True)        
            refer_tokens = self.get_image_token(refer)
            gt_tokens = self.get_image_token(gt)
            sample_refer_tokens = self.get_image_token(sample_refer)
            sample_gt_tokens = self.get_image_token(sample_gt)
            token_indices = torch.cat((sample_refer_tokens, sample_gt_tokens, refer_tokens, gt_tokens), dim=-1)
        elif data_type == 'vp':
            sample_refer, sample_gt, refer, gt = data['sample_refer'], data['sample_gt'], data['refer'], data['gt']
            refer = refer.to(device, non_blocking=True)
            gt = gt.to(device, non_blocking=True)
            sample_refer = sample_refer.to(device, non_blocking=True)
            sample_gt = sample_gt.to(device, non_blocking=True)  
            ...
            sample_refer, sample_gt, refer, gt = data['support_mask'], data['support_img'], data['query_mask'], data['query_img']
            refer = refer.to(device, non_blocking=True)
            gt = gt.to(device, non_blocking=True)
            sample_refer = sample_refer.to(device, non_blocking=True)
            sample_gt = sample_gt.to(device, non_blocking=True)  
            ...
            # refer_tokens = refer_tokens.view(B, int(sqrt(N)), int(sqrt(N)))
            # gt_tokens = gt_tokens.view(B, int(sqrt(N)), int(sqrt(N)))
            # sample_refer_tokens = sample_refer_tokens.view(B, int(sqrt(N)), int(sqrt(N)))
            # sample_gt_tokens = sample_gt_tokens.view(B, int(sqrt(N)), int(sqrt(N)))

            # samples_tokens = torch.cat([sample_refer_tokens, sample_gt_tokens], dim=1)
            # gts_tokens = torch.cat([refer_tokens, gt_tokens], dim=1)
            token_indices = torch.cat((sample_refer_tokens, sample_gt_tokens, refer_tokens, gt_tokens), dim=-1)
        else:
            refer, gt, samples = data['m_img'], data['img'], data['samples']
            refer.to(device, non_blocking=True)
            gt.to(device, non_blocking=True)
            samples.to(device, non_blocking=True)
            refer_tokens = self.get_image_token(refer)
            gt_tokens = self.get_image_token(gt)
            samples_tokens = self.get_image_token(samples)

            token_indices = torch.cat((samples_tokens, refer_tokens, gt_tokens), dim=-1)
        return token_indices

    def forward_encoder(self, data, device):
        # tokenization
        with torch.no_grad():
            token_indices = self.get_input(data, device)
    ...
    def forward_loss(self, gt_indices, logits, mask, names):
        ...
        loss_all = (loss * mask[:, 1:]).sum() / mask[:, 1:].sum()  # mean loss on removed patches
        loss_gt = (loss[:, seq_len*3//4:] * mask[:, seq_len*3//4 + 1:]).sum() / mask[:, seq_len*3//4 + 1:].sum()

        loss_dict = {}
        loss_dict['CE_all'] = loss_all
        loss_dict['CE_gt'] = loss_gt
        ...
        for i, name in enumerate(names):
            loss_data = (loss[i] * mask[i, 1:]).sum() / mask[i, 1:].sum() 
            # print(loss_data)
            loss_temp_dict[name] += loss_data.item()
            loss_num_dict[name] += 1

        for i, name in enumerate(dataset_names):
            if loss_num_dict[name] == 0:
                loss_data_dict[name] = 0
            else:
                loss_data_dict[name] = loss_temp_dict[name] / loss_num_dict[name]

        del loss_temp_dict, loss_num_dict

        return loss_dict, loss_data_dict

    def forward(self, data, device):
        names = data['name']
        latent, gt_indices, token_drop_mask, token_all_mask = self.forward_encoder(data, device)
        logits = self.forward_decoder(latent, token_drop_mask, token_all_mask)
        loss, loss_data = self.forward_loss(gt_indices, logits, token_all_mask, names)
        return loss, loss_data, token_all_mask
