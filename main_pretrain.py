...
from engine_pretrain import my_train_one_epoch, my_validate
...
def get_args_parser():
    ...
    parser.add_argument('--data_path', default='./data/imagenet', type=str,
                        help='dataset path')
    parser.add_argument('--data_type', default='CVF', type=str, 
                        choices=['CVF', 'inpaint', 'color', 'caption', 'vp'])
...
def main(args):
    ...
    train_list = os.path.join(args.data_path, 'imagenetcolor_train.txt')
    val_path = os.path.join(args.data_path, 'val')
    val_list = os.path.join(args.data_path, 'imagenetcolor_val.txt')

    # train_vp_list = '/mnt/v-dsheng/data/visual_prompt_dataset/train.txt'
    # val_vp_list = '/mnt/v-dsheng/data/visual_prompt_dataset/val.txt'
    train_vp_list = '/mnt/v-dsheng/data/visual_prompt_dataset/train_1w_name.txt'
    val_vp_list = '/mnt/v-dsheng/data/visual_prompt_dataset/val_name.txt'

    if args.data_type == 'CVF':
        dataset_train = datasets.ImageFolder(train_path, transform=transforms_train)
        dataset_val = datasets.ImageFolder(val_path, transform=transforms_train)
    elif args.data_type == 'vp':
        ...
        mask_transform = transforms.Compose([
            ...
            transforms.ToTensor()])
        dataset_train = MyVisualPromptDataset(train_vp_list, image_transform, mask_transform)
        dataset_val = MyVisualPromptDataset(val_vp_list, image_transform, mask_transform)
        ...
        dataset_train = MyInpaintingTrainDataset(train_path, transforms_train, args.samples_num)
        dataset_val = MyInpaintingTrainDataset(val_path, transforms_train, args.samples_num)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and (epoch % 40 == 0 or epoch + 1 == args.epochs):
            my_validate(model, data_loader_val, device, epoch, log_writer=log_writer, args=args)
            misc.save_model(
...
if __name__ == '__main__':
    ...
    desc = args.model
    # model_size = args.config.split('.')[0].split('_')[-1]
    # desc += f'-{model_size}'
    ...
    args.output_dir = os.path.join(args.output_dir, f'{cur_run_id:05d}-{desc}')
